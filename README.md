<!-- ## WARNING: Having file mods installed while playing with this custom game *could cause issues*! One of the most well known ones is unbreakable crates that occur while having Fant mod installed. -->

![preview](https://raw.githubusercontent.com/Vajdani/Raft-Mechanic-Game-Mode/refs/heads/main/preview.jpg)

# This Mod Adds Features From Axolot Games' Other Title, [Raft](https://store.steampowered.com/app/648800/Raft/)!
**Play the game in a completely new and different way than ever before!**

# Media
<iframe width="100%" height="500" alt="embed_1" src="https://www.youtube.com/embed/PUUflpzfTiQ"></iframe> 

![embed_2](https://images.steamusercontent.com/ugc/1877451307941210897/19C58BD871E3213EA52858B9C77F8D51CEFB307E/)
![embed_3](https://images.steamusercontent.com/ugc/1877451307941212372/595AC977DBE9C0CD41A37170235E3E813DBDFAF6/)
![embed_4](https://images.steamusercontent.com/ugc/1877451307941212916/976C8C103519135AD8AE40909A3F257EC886F7EB/)
![embed_5](https://images.steamusercontent.com/ugc/1877451307941213485/EEE0E3D6289EC0394E4B7D14B3BFF118B107B4B5/)
![embed_6](https://images.steamusercontent.com/ugc/1877451307941213960/4342B4399A4F0A96549933A31FF31F014D767924/)

<img src="https://images.steamusercontent.com/ugc/1910107854874128800/69C6E2743E6C83145728D4668F37D714FE820340/" alt="embed_7" width="100%">
<img src="https://images.steamusercontent.com/ugc/1910107854874129304/C2949E6CB5FF7253E36E7C00050313E41B5060F2/" alt="embed_8" width="100%">
<img src="https://images.steamusercontent.com/ugc/1910107854874129712/358FC318D7B984A3E9C86296BB1F2DA1BAD069F2/" alt="embed_9" width="100%">

# INSTALLATION:

**1. Subscribe to this mod**
**2. Launch Scrap Mechanic and press the custom game button above the creative game loader**
**3. Select this mod from the list and then load into the world and play!**

**NOTE: You do not have to manually install this mod**

# For information on importing your old raft into the new world , read the guide [here](https://steamcommunity.com/workshop/filedetails/discussion/2807590049/3391786569663862852/)!

Features:
- **A story with quests that you have to complete to unlock new parts**
- The world is a beautiful ocean to explore with new and custom tiles!
- A sharkbot that lurks beneath the water...
- New tools and weapons:
  - Axe
  - Pickaxe
  - Harpoon Gun
  - Pulling Hook
  - Fishing Rod
- New logic blocks:
  - WASD Converter
  - Water Sensor
- New parts:
  - Propeller(small and big)
  - Sail & Wind Sock
  - Farms you can PLACE down 
  - Air Tank
  - Water Purifier
  - Apiary
  - Collector to collect ocean junk
  - Creation Rotator
  - *And many more!*
- Adjusted some things:
  - Loot has been adjusted to be less grindy
  - Resources such as Component Kits can be crafted now
  - Farmbots no longer drop warehouse keys

# Check out the [source code](https://github.com/RaftMechanic/Raft-Mechanic-Game-Mode)!

# For a list of compatible mods, [look here](https://steamcommunity.com/sharedfiles/filedetails/?id=2816900681)!

Translations:
- Italian - [Fagiano](https://steamcommunity.com/id/Fagiano0)
- French - [Corvos](https://steamcommunity.com/id/corv0s)
- German - [DavidWarNichtVerfügbar](https://steamcommunity.com/profiles/76561198824292425)
- Polish - [Cookiez_1](https://steamcommunity.com/profiles/76561199100233083/) & [Dart Frog](https://steamcommunity.com/id/DartFrogYT/)
- Brazilian - [ErickXavierS](https://steamcommunity.com/id/erickxavier/)
- Russian - [Oleg4260](https://steamcommunity.com/id/Oleg4260/)

If you enjoy this mod, don't forget to rate it!